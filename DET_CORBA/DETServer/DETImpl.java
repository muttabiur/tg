...
class DETImpl extends DETPOA 
{
    ...
    public String trapNameGenericGlobal = null ;
    public String trapNameSpecificGlobal = null ;
    ...
    public  int QTY = 0;
    // public static final double BALANCE = 00;
	...
	synchronized(lockItm){
	    ...
	    if ((bal == 0) ||(bal < 0))
	    {
		System.out.println("How much Balance this Agent Need:");
	...
	synchronized(lockItm){
	   ...
	   // String pr=df.format(Double.valueOf(itm.getPrice()));
	    ...
	    // if balance is 0 then sends message to the agent
	    int quantity = itm.getQty();
	    if ((quantity == 0) ||(quantity < 0))
	    {
		System.out.println("How much Quantity this Agent Need:");
		Scanner key = new Scanner(System.in);
		System.out.println("Enter a New Quantity:");
		int qutity = key.nextInt();
		//double balance = key.nextDouble();
	...
	Manager m = new Manager();
	m.MessagePassed("The Quantity Set up is done Sucessfully:");
	...
	synchronized(lockItm){
	   ...
	   // String pr=df.format(Double.valueOf(itm.getPrice()));
		...
		try{
		    ...
		    detImpl =  DETHelper.narrow(ncRef.resolve_str(name));
		    //Calling traptype.
		    detImpl.addGenericTrapType(0);
		    detImpl.addSpecificTrapType(0);
	...
	synchronized(lockItm){
	    ...
	    System.out.println("");
	    System.out.println("The Current Information about ServerComputer After Some transactions Happened!");
	...
	case 0 : 
	    trapName = "coldStart" ;
	    signalTrapTypeGeneric(trapName);
	    //System.out.println("The Cold Start is:" + trapName);
	    break ;
	...
	default:
	    System.out.println("Invalid Generic Trap Type:");
	break ;
	...
	trapNameGenericGlobal = trapType ;

    }

    public String getTrapNameGeneric() {

	return trapNameGenericGlobal ;
    }
	...
	switch(type) {
	case 0 : 
	    trapName = "BalanceNegative" ;
	    signalTrapTypeSpecific(trapName);
	    //System.out.println("The Cold Start is:" + trapName);
	    break ;

	case 1:
	    trapName = "QuantityEqualToZero" ;
	    signalTrapTypeSpecific(trapName);
	    break ;

	case 3:
	    trapName = "ReducedPrice" ;
	    signalTrapTypeSpecific(trapName);
	    break ;


	case 4:
	    trapName = "IncreasedPrice" ;
	    signalTrapTypeSpecific(trapName);
	    break ;


	default:
	    System.out.println("Invalid Specific Trap Type:");
	break ;
	...
	trapNameSpecificGlobal = trapType ;

    }

    public String getTrapNameSpecific() {

	return trapNameSpecificGlobal ;
    }
	...
	synchronized(lockItm){
	   ...
	   // String pr=df.format(Double.valueOf(itm.getPrice()));
	    //int quantity =df.format(Double.valueOf(itm.getQty()));
	    ...
	    if ((quantity == 0) ||(quantity < 0))
	    {
		m.snmpTrapMessageReceivedForQuantity(0);
